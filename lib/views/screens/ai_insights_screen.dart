...
class _AiInsightsScreenState extends ConsumerState<AiInsightsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final humidity = ds.weatherForecast.isNotEmpty
        ? ds.weatherForecast.first.humidity
        : 0.0;
    final temp =
        ds.summary?.temperature ??
        (ds.weatherForecast.isNotEmpty
            ? ds.weatherForecast.first.dayTemp
            : 0.0);
    final rain =
        ds.summary?.rainProbability ??
        (ds.weatherForecast.isNotEmpty
            ? ds.weatherForecast.first.precipChance
            : 0.0);

    final droughtRisk = _clamp(1.0 - ndvi * 1.2 + (rain == 0 ? 0.3 : 0));
    final pestRisk = _clamp(humidity / 100 * 0.8 + (ndvi < 0.45 ? 0.3 : 0));
    final yieldScore = _clamp(
      ndvi * 0.7 +
          (humidity > 50 ? 0.15 : 0) +
          (rain > 0 ? 0.1 : 0) +
          (temp < 35 ? 0.05 : 0),
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                _cropAdvisorySection(ds),
                const SizedBox(height: 16),
                _cropCalendarSection(ds),
                const SizedBox(height: 16),
                _riskAlertsCard(droughtRisk, pestRisk, humidity, temp, rain),
                const SizedBox(height: 16),
                _soilReportButton(context),
                const SizedBox(height: 16),
                _recommendationsCard(ds, ndvi, droughtRisk, pestRisk),
  ...
  Widget _scanStat(String label, String value, Color color) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(
        label,
  ...
  Widget _predictionBarsCard(
    ...
    final metrics = [
      ...
      {'label': '🌱 Soil Health', 'value': soil, 'color': AppColors.goldAccent},
      {'label': '🔥 Drought Risk', 'value': drought, 'color': Colors.redAccent},
      {'label': '🐛 Pest Risk', 'value': pest, 'color': Colors.orangeAccent},
    ];
  ...
  // ── Crop Advisory from API ────────────────────────────────────────────
  Widget _cropAdvisorySection(DashboardState ds) {
  ...
  // ── Crop Calendar ─────────────────────────────────────────────────────
  Widget _cropCalendarSection(DashboardState ds) {
    if (ds.cropCalendar.isEmpty) return const SizedBox.shrink();
    return GlassCard(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      child: Column(
        ...
        children: [
          ...
          ...ds.cropCalendar.map(
            (c) => Container(
              ...
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: AppColors.secondaryAccent2.withOpacity(0.06),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(
                  color: AppColors.secondaryAccent2.withOpacity(0.2),
                ),
              ),
              child: Row(
                children: [
  ...
  Widget _riskAlertsCard(
    ...
    if (pest > 0.55)
      alerts.add({
        'icon': '🐛',
        'label': 'Pest Warning',
        'detail': 'Apply organic pesticide',
    ...
    if (humidity > 80)
      alerts.add({
        'icon': '🍄',
        'label': 'Fungal Risk',
        'detail': 'High humidity ${humidity.toInt()}%',
  ...
  Widget _soilReportButton(BuildContext context) {
    return SizedBox(
      width: double.infinity,
      child: OutlinedButton.icon(
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const SoilReportScreen()),
        ),
        icon: const Icon(Icons.terrain, color: AppColors.goldAccent, size: 18),
        label: const Text(
          'VIEW SOIL REPORT',
          style: TextStyle(
            color: AppColors.goldAccent,
  ...
  Widget _recommendationsCard(
    ...
    final apiRecommendations = ds.summary?.recommendations ?? const <String>[];
    final recs = <Map<String, String>>[];

    for (final item in apiRecommendations) {
      recs.add({'icon': '📌', 'action': item, 'priority': 'API'});
    }
    if (recs.isEmpty) {
      if (drought > 0.4) {
        recs.add({
          'icon': '💧',
          'action': 'Irrigation likely needed based on low moisture risk.',
          'priority': 'Urgent',
        });
      }
      if (pest > 0.4) {
        recs.add({
          'icon': '🐛',
          'action': 'Monitor pest activity and apply suitable controls.',
          'priority': 'Soon',
        });
      }
      if (ndvi < 0.55) {
        recs.add({
          'icon': '🌱',
          'action': 'Vegetation health is moderate. Plan corrective nutrition.',
          'priority': 'This week',
        });
      }
    }
